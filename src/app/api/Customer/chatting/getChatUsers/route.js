...
export async function GET(req) {
  try {
   ...
   for (const msg of messages) {
     ...
     if (!isSender || msg.isRead === false) {
      //  console.log(
      //    `Unread message found for key: ${key}`,
      //    msg.message,
      //    msg.isRead,
      //    msg.senderId.toString(),
      //    "->",
  ...
  for (const [key, value] of chatMap.entries()) {
    ...
    // Query all types
    const [customers, employees, branchAdmins] = await Promise.all([
      ...
      // BranchAdmin.find({ _id: { $in: userGroups.branchAdmin } }).select(
      //   "_id firstName lastName image"
      // ),
    ]);
    ...
    // const allUsers = [...customers, ...employees, ...branchAdmins];
